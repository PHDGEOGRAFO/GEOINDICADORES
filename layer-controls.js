  ...
  async function install(){
    ...
    try{
      const [barrios,territorios,comuna]=await Promise.all([
        fetch('data/limite_barrios.geojson?v=0.9.9').then(r=>r.json()),
        fetch('data/limite_territorios_pladeco.geojson?v=0.9.9').then(r=>r.json()),
        fetch('data/limite_comuna.geojson?v=0.9.9').then(r=>r.json())
      ]);
      territorios.features.forEach(f=>{f.properties.TERRITORIO=f.properties.SECTORES_T;});
      ...
      try{const mz=await fetch('data/limite_manzana_censal.geojson?v=0.9.9').then(r=>r.ok?r.json():Promise.reject());add('limites-manzanas',mz,{'line-color':'#8ca0b3','line-width':0.65,'line-opacity':0.65},false);}catch(e){console.warn('GeoJSON canónico de límite de manzana pendiente de publicación');}
